...
// Helper function to create a transaction with TransactonPurchase struct so it can be used in CreateTransaction
func (s *SmartContract) CreateTransactionPurchase(ctx contractapi.TransactionContextInterface, transactionID string, amount float64, location string, reciever string, sender string, status string, timestamp string, amountInGrams float64, amountInMiligramsOfTotalTHC float64, category string, cost float64, fedTax float64, name string, stateTax float64, thcPercent float64, weFee float64, typeOfTransaction string) error {
	transactionPurchase := TransactionPurchase{
		AmountInGrams:               amountInGrams,
		AmountInMiligramsOfTotalTHC: amountInMiligramsOfTotalTHC,
		Category:                    category,
		Cost:                        cost,
		FedTax:                      fedTax,
		Name:                        name,
	...
	err := s.CreateTransaction(ctx, transactionID, amount, location, reciever, sender, status, timestamp, transactionPurchase, typeOfTransaction)
