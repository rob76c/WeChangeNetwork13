...
function create_local_MSP() {
  ...
  create_orderer_local_MSP orderer orderer1
  create_orderer_local_MSP orderer orderer2
  create_orderer_local_MSP orderer orderer3

  create_peer_local_MSP wechange peer1 $ORG1_NS
  create_peer_local_MSP wechange peer2 $ORG1_NS

  create_peer_local_MSP wechangeappusers peer1 $ORG2_NS
  create_peer_local_MSP wechangeappusers peer2 $ORG2_NS
...
function launch_orderers() {
  ...
  apply_template kube/orderer/orderer-orderer1.yaml $ORG0_NS
  apply_template kube/orderer/orderer-orderer2.yaml $ORG0_NS
  apply_template kube/orderer/orderer-orderer3.yaml $ORG0_NS

  kubectl -n $ORG0_NS rollout status deploy/orderer-orderer1
  kubectl -n $ORG0_NS rollout status deploy/orderer-orderer2
  kubectl -n $ORG0_NS rollout status deploy/orderer-orderer3
...
function launch_peers() {
  ...
  apply_template kube/wechange/wechange-peer1.yaml $ORG1_NS
  apply_template kube/wechange/wechange-peer2.yaml $ORG1_NS
  apply_template kube/wechangeappusers/wechangeappusers-peer1.yaml $ORG2_NS
  apply_template kube/wechangeappusers/wechangeappusers-peer2.yaml $ORG2_NS

  kubectl -n $ORG1_NS rollout status deploy/wechange-peer1
  kubectl -n $ORG1_NS rollout status deploy/wechange-peer2
  kubectl -n $ORG2_NS rollout status deploy/wechangeappusers-peer1
  kubectl -n $ORG2_NS rollout status deploy/wechangeappusers-peer2
