...
function init_tls_cert_issuers() {
  ...
  kubectl -n $ORG2_NS apply -f kube/wechangeappusers/wechangeappusers-tls-cert-issuer.yaml
  ...
  kubectl -n $ORG2_NS wait --timeout=30s --for=condition=Ready issuer/wechangeappusers-tls-cert-issuer
...
function launch_ECert_CAs() {
  ...
  apply_template kube/wechangeappusers/wechangeappusers-ca.yaml $ORG2_NS

  kubectl -n $ORG0_NS rollout status deploy/orderer-ca
  kubectl -n $ORG1_NS rollout status deploy/wechange-ca
  kubectl -n $ORG2_NS rollout status deploy/wechangeappusers-ca
...
function enroll_bootstrap_ECert_CA_users() {
  ...
  enroll_bootstrap_ECert_CA_user wechangeappusers $ORG2_NS
