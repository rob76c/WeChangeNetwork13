...
function init_storage_volumes() {
  ...
  cat kube/pvc-fabric-wechangeappusers.yaml | envsubst | kubectl -n $ORG2_NS create -f - || true
...
function load_org_config() {
  ...
  kubectl -n $ORG2_NS delete configmap wechangeappusers-config || true
  ...
  kubectl -n $ORG2_NS create configmap wechangeappusers-config --from-file=config/wechangeappusers
...
function apply_k8s_builders() {
  ...
  apply_template kube/wechangeappusers/wechangeappusers-install-k8s-builder.yaml $ORG2_NS

  kubectl -n $ORG1_NS wait --for=condition=complete --timeout=60s job/wechange-install-k8s-builder
  kubectl -n $ORG2_NS wait --for=condition=complete --timeout=60s job/wechangeappusers-install-k8s-builder
