...
function register_org_admins() {
  ...
  register_org_admin orderer ordereradmin ordereradminpw
  register_org_admin wechange wechangeadmin wechangeadminpw
  register_org_admin wechangeappusers wechangeappusersadmin wechangeappusersadminpw
...
function enroll_org_admins() {
  ...
  enroll_org_admin orderer  orderer ordereradmin ordereradminpw
  enroll_org_admin peer     wechange wechangeadmin wechangeadminpw
  enroll_org_admin peer     wechangeappusers wechangeappusersadmin wechangeappusersadminpw
...
function create_channel_MSP() {
  ...
  create_channel_org_MSP orderer orderer $ORG0_NS
  create_channel_org_MSP wechange peer $ORG1_NS
  create_channel_org_MSP wechangeappusers peer $ORG2_NS

  extract_orderer_tls_cert orderer orderer1
  extract_orderer_tls_cert orderer orderer2
  extract_orderer_tls_cert orderer orderer3
...
function create_genesis_block() {
  ...
  cat ${PWD}/config/orderer/configtx-template.yaml | envsubst > ${TEMP_DIR}/configtx.yaml
...
function join_channel_orderers() {
  ...
  join_channel_orderer orderer orderer1
  join_channel_orderer orderer orderer2
  join_channel_orderer orderer orderer3
...
function join_channel_peers() {
  join_org_peers wechange
  join_org_peers wechangeappusers
}
...
function join_channel_peer() {
  ...
  peer channel join \
    ...
    --orderer     orderer-orderer1.${DOMAIN} \
    --connTimeout ${ORDERER_TIMEOUT} \
    --tls         \
    --cafile      ${TEMP_DIR}/channel-msp/ordererOrganizations/orderer/orderers/orderer-orderer1/tls/signcerts/tls-cert.pem
}
