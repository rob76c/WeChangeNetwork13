...
func (v *IssueCashView) Call(context view.Context) (interface{}, error) {
	...
	//Add a log to see the recipient node
	logger.Infof("recipient node: %s", node)
	rec := view.Identity(v.Recipient)
	//Add a log to see the recipient
	logger.Infof("recipient: %s", rec)
	eps := viewregistry.GetEndpointService(context)
	// Check if the recipient is bound to the recipient node
